...
/*
    ...
    var audioUrl = document.getElementById('audioPlayer').currentSrc;
    var audioFile = null;

	fetch(audioUrl)
	    .then(response => response.blob())
	    .then(blob => {
	        console.log('Blob do áudio:', blob);

	        // Se quiser criar um "File" artificial:
	        audioFile = new File([blob], 'audio.mp3', { type: blob.type });
	        console.log('Arquivo criado:', audioFile);
	        console.log(audioFile instanceof Blob); // deve ser true
    ...
    .catch(err => console.error('Erro ao baixar o áudio:', err));

    console.log("É BLOB:", audioFile instanceof Blob); // deve ser true
